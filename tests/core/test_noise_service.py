...
def _make_clean_png(tmp_path):
    """Smooth gradient saved as lossless PNG — dominated by low frequencies → high SNR."""
...
def _make_noisy_png(tmp_path):
    """Random pixel noise saved as lossless PNG — flat frequency spectrum → low SNR."""
...
def test_compute_score_clean_higher_than_noisy(tmp_path):
    ...
    _make_clean_png(tmp_path)
    _make_noisy_png(tmp_path)
    svc = NoiseService()
    clean = svc.compute_score(str(tmp_path), "clean.png")
    noisy = svc.compute_score(str(tmp_path), "noisy.png")
...
def test_compute_score_returns_non_negative_float(tmp_path):
    ...
    _make_clean_png(tmp_path)
    svc = NoiseService()
    score = svc.compute_score(str(tmp_path), "clean.png")
    assert isinstance(score, float)
    assert score >= 0.0


def test_is_noisy_fixed_below_threshold(tmp_path):
    """Random-noise image scores near 0; default threshold 0.5 should classify it as noisy."""
    from app.core.noise_service import NoiseService
    _make_noisy_png(tmp_path)
    svc = NoiseService()
    score = svc.compute_score(str(tmp_path), "noisy.png")
    assert score is not None
    assert svc.is_noisy_fixed(score, threshold=0.5) is True
...
def test_compute_score_reads_unicode_filename(tmp_path):
    ...
    for i in range(50):
        arr[i, :] = int(i * 255 / 49)
    encoded = cv2.imencode(".png", arr)[1]
    path = tmp_path / "60919-0012_調整大小.png"
