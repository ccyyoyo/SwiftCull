...
def _make_image_with_line(tmp_path, filename, angle_deg):
    """Draw a long high-contrast line at the given angle on a gray background."""
...
def test_compute_skew_returns_float_for_level_image(tmp_path):
    from app.core.horizon_service import HorizonService
    _make_image_with_line(tmp_path, "level.jpg", 0.0)
    svc = HorizonService()
    result = svc.compute_skew(str(tmp_path), "level.jpg")
    assert isinstance(result, float)


def test_compute_skew_tilted_image_returns_nonzero_angle(tmp_path):
    from app.core.horizon_service import HorizonService
    _make_image_with_line(tmp_path, "tilted.jpg", 15.0)
    svc = HorizonService()
...
def test_compute_skew_returns_none_for_solid_image(tmp_path):
    """A solid-color image has no edges, so HoughLinesP finds nothing."""
    from app.core.horizon_service import HorizonService
...
def test_skew_state_exact_threshold_is_level():
    from app.core.horizon_service import HorizonService
    from app.core.models import Photo
    photo = Photo(id=1, relative_path="a.jpg", filename="a.jpg",
                  file_size=1, horizon_skew=1.0)
    assert HorizonService.skew_state(photo, level_threshold=1.0) == "level"
